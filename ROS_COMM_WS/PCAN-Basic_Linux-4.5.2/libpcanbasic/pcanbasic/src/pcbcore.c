 ...
 * $Id: pcbcore.c 12545 2021-09-24 15:10:21Z Fabrice $
 *
 ...
 */
static void pcanbasic_get_hw(TPCANHandle channel, enum pcaninfo_hw *hw, __u32 *index);
...
void pcanbasic_get_hw(TPCANHandle channel, enum pcaninfo_hw *hw, __u32 *index) {
...
struct pcaninfo* pcanbasic_get_device(TPCANHandle channel, __u32 hwtype, __u32 base, __u32 irq, TPCANStatus* status) {
	...
	TPCANStatus sts = PCAN_ERROR_ILLHW;
...
TPCANStatus pcanbasic_set_hwtimestamp_mode(pcanbasic_channel* pchan) {
	...
	if (ires < 0) {
		pcanlog_log(LVL_NORMAL, "Failed to set PCANFD_OPT_HWTIMESTAMP_MODE to PCANFD_OPT_HWTIMESTAMP_COOKED(%d) (sts=%d)", param, ires);
		/* fallback to SW timestamps */
		param = PCANFD_OPT_HWTIMESTAMP_OFF;
		ires = pcanfd_set_option(pchan->fd, PCANFD_OPT_HWTIMESTAMP_MODE, &param, sizeof(param));
		pcanlog_log(LVL_DEBUG, "Switching PCANFD_OPT_HWTIMESTAMP_MODE to PCANFD_OPT_HWTIMESTAMP_OFF(%d) (sts=%d)", param, ires);
	}
...
TPCANStatus pcanbasic_read(
	...
	if (sts == PCAN_ERROR_OK) {
		...
		if (timestamp != NULL) {
			...
			__u64 millis = ((__u64) t.tv_sec) * 1000 + t.tv_usec;
...
#pragma GCC diagnostic pop
	...
	case PCAN_FIRMWARE_VERSION:
		...
		break;
#if defined(PCAN_HW_SELF_RECEPTION)
	case PCAN_HW_SELF_RECEPTION:
...
TPCANStatus pcanbasic_set_value(
	...
	case PCAN_LOG_CONFIGURE:
		...
		break;
	case PCAN_LOG_TEXT:
		pcblog_write(buffer, strnlen(buffer, len));
		goto pcanbasic_set_value_exit_ok;
		break;
	case PCAN_RECEIVE_STATUS:
		/* can be called with an uninitialized channel */
	...
	case PCAN_HW_SELF_RECEPTION:
		...
		memcpy(&pchan->rcv_echo_status, buffer, size);
		break;
#endif
	default:
		sts = PCAN_ERROR_ILLPARAMTYPE;
