...
/* if defined, then closing a device waits for the Tx fifo to be empty during
 * a time that is estimated, according to the content of the Tx fifo and the
 * nominale bitrate, waiting for it if and only if bus state is ACTIVE.
 * See also: PCANFD_OPT_LINGER, PCANFD_OPT_LINGER_AUTO, PCANFD_OPT_LINGER_NOWAIT
 */
#define PCAN_USES_NEW_TXFLUSH_METHOD
...
#ifdef PCAN_USES_NEW_TXFLUSH_METHOD
	...
	case 1:
...
#else /* PCAN_USES_NEW_TXFLUSH_METHOD */
		...
		if (pcan_task_can_wait()) {
			...
			pr_info(DEVICE_NAME ": %s CAN%u preparing to wait: "
				...
				dev->adapter->name, dev->can_idx+1,
				dev->is_plugged,
				pcan_fifo_items_count(&dev->writeFifo),
				dev->locked_tx_engine_state, dev->bus_state,
				MAX_WAIT_UNTIL_CLOSE);
#endif

			/* Now, wait for the tx fifo to be empty */
			pcan_event_wait_timeout(dev->out_event,
...
#endif /* PCAN_USES_NEW_TXFLUSH_METHOD */
