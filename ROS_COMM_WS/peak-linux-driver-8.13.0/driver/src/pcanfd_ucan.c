...
#if 0
	if (reset_to_us % 1000)
		pcan_usleep(reset_to_us);
	else
		pcan_msleep_interruptible(reset_to_us / 1000);
#endif
...
int ucan_handle_msgs_buffer(struct ucan_engine *ucan, void *buffer_addr,
			    int buffer_len)
{
	const u8 *msg_end = buffer_addr + buffer_len;
	...
	for (; ;) {
		...
		if ((len_handled + msg_size) > buffer_len) {

			/* fragmented msg */
			ucan->frag_size = buffer_len - len_handled;
			memcpy(ucan->frag_rec, msg_ptr, ucan->frag_size);

#ifdef DEBUG
			pr_warn(DEVICE_NAME
				": msg #%u fragmented: %u/%u bytes pending "
				"(buffer_len=%d len_handled=%d)\n",
				msg_nb+1, ucan->frag_size, msg_size,
				buffer_len, len_handled);
#endif
			break;
		}
