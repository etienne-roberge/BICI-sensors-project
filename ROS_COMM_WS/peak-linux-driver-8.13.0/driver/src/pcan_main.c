				...
				struct device_attribute *attr, char *buf)
{
	...
	if (pbt)
		return show_u32(buf, pbt->sample_point * 10);
#endif
	return show_u32(buf,
...
static void pcan_push_bus_load_ind(struct timer_list *t)
{
	...
	if ((bl_diff >= 50) || (bl_diff <= -50) ||
		...
		/* do update prev_bus_load only if we're certain that the
				      ...
				      int err)
{
	...
	/* To avoid filing kernel log buufer, display a msg *ONLY* if
...
int pcan_chardev_rx(struct pcandev *dev, struct pcanfd_rxmsg *rx)
{
	...
	default:
		pr_info(DEVICE_NAME
			...
			rx->msg.data_len, rx->msg.data[0], rx->msg.data[1],
			rx->msg.data[2], rx->msg.data[3]);
		break;
	}
#endif
...
int pcan_post_bus_state(struct pcandev *dev)
{
	...
	/* to be sure that msg will be put into Rx fifo: */
...
/*
 * get CAN frame from the the device Tx fifo.
 */
...
void pcan_handle_error_active(struct pcandev *dev, struct pcanfd_rxmsg *rx)
{
	...
	pr_info(DEVICE_NAME ": %s(%s CAN%u): bus=%u rx=%u/tx=%u (@rx=%p)\n",
		__func__, dev->adapter->name, dev->can_idx+1, bus_state,
				...
				int err_ctrlr)
{
	...
	default:
		...
		pr_err(DEVICE_NAME ": %s CAN%u: controller internal error %d\n",
		       dev->adapter->name, dev->can_idx+1, err_ctrlr);
		break;
	}
...
static struct pcanfd_options pcan_def_opts[PCANFD_OPT_MAX] = 
{
	...
	[PCANFD_OPT_LINGER] = {
		.req_size = sizeof(int),	/* always 4 bytes log */
		.get = pcan_get_linger,
		.set = pcan_set_linger
	},
};
...
int init_module(void)
{
	...
	if (txqhiwat < PCANFD_TX_HIWAT_MIN)
		txqhiwat = PCANFD_TX_HIWAT_MIN;

	else if (txqhiwat > PCANFD_TX_HIWAT_MAX)
		txqhiwat = PCANFD_TX_HIWAT_MAX;
