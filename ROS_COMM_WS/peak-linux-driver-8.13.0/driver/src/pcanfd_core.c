...
int pcanfd_ioctl_set_init(struct pcandev *dev, struct pcanfd_init *pfdi)
{
	...
	if (dev->nOpenPaths > 1) {
		pr_err(DEVICE_NAME ": %s CAN%u can't be initialized twice\n",
		       dev->adapter->name, dev->can_idx+1);
	...
	if (!(dev->features & PCAN_DEV_BUSLOAD_RDY) ||
		...
		return pcan_post_bus_state(dev);
#else
		return 0;
#endif
	}
...
int pcanfd_ioctl_reset(struct pcandev *dev, unsigned long flags)
{
	...
	pr_info(DEVICE_NAME ": %s(%s CAN%u, flgs=%08lxh)\n",
		__func__, dev->adapter->name, dev->can_idx+1, flags);
#endif
				      ...
				      int err)
{
	...
	if (err >= 0) {
		...
		pr_info(DEVICE_NAME
			...
			__func__, dev->adapter->name, dev->can_idx+1, err);
#endif
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(%s CAN%u tx_engine_state=%u)\n",
		__func__, dev->adapter->name, dev->can_idx+1,
