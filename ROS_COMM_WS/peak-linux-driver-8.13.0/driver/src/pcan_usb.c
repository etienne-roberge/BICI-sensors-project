...
static int pcan_usb_getBTR0BTR1(struct pcandev *dev, u16 *pwBTR0BTR1)
{
	...
	err = pcan_usb_getcontrol_urb(dev, PCAN_USB_CMD_BITRATE, PCAN_USB_GET,
				      &param0, &param1,
				      NULL, NULL);
...
static int pcan_usb_getQuartz(struct pcandev *dev, u32 *pdwQuartzHz)
{
	...
	err = pcan_usb_getcontrol_urb(dev, PCAN_USB_CMD_CLOCK, PCAN_USB_GET,
				      &param0, NULL, NULL, NULL);
							...
							int *pnDataLength)
{
	...
	*ptr++ = 2;
	...
	while ((ptr - ucMsgPtr) < nBufferTop) {
		...
		if (usb_if->ucRevision < 41)
			tx.msg.flags &=
				~(PCANFD_MSG_SLF|MSGTYPE_SINGLESHOT);
#endif
